...
const toOrderItem = (item: OrderItemRow) => ({
  id: item.id,
...
const toOrder = (order: OrderRow, items: OrderItemRow[]) => ({
  id: order.id,
  status: order.status,
...
ordersRouter.get('/', requireAuth, async (req, res) => {
  ...
  for (const order of orders) {
    const items = await all<OrderItemRow>('SELECT * FROM order_items WHERE order_id = ? ORDER BY created_at ASC', [
      order.id,
    ]);
...
ordersRouter.post('/', requireAuth, async (req, res) => {
  ...
  const menuItems = await all<MenuItemRow>(
  ...
  const quantityMap = new Map(items.map((item) => [item.menuItemId, item.quantity] as const));
  const subtotal = menuItems.reduce((sum, item) => sum + Number(item.price) * (quantityMap.get(item.id) ?? 0), 0);
  const total = subtotal + deliveryFee + tax;

  if (addressId) {
    const address = await get<AddressRow>('SELECT * FROM addresses WHERE id = ? AND user_id = ?', [
      addressId,
      req.auth?.userId ?? '',
    ]);
  ...
  const createdOrder = await transaction(async (db) => {
    ...
    const deleteCart = db.prepare(
      `DELETE FROM cart_items WHERE user_id = ? AND menu_item_id IN (${menuItemIds.map(() => '?').join(',')})`,
    );
  ...
  const restaurant = await get<{ name: string; image_url: string | null }>(
    'SELECT name, image_url FROM restaurants WHERE id = ?',
    [restaurantId],
  );

  const itemsRows = await all<OrderItemRow>('SELECT * FROM order_items WHERE order_id = ? ORDER BY created_at ASC', [
    createdOrder.id,
  ]);
