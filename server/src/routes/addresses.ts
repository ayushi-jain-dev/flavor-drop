...
const toAddress = (address: AddressRow) => ({
  id: address.id,
  label: address.label,
  line1: address.line1,
  line2: address.line2,
...
const loadAddresses = async (userId: string) => {
  const addresses = await all<AddressRow>(
...
addressesRouter.post('/', requireAuth, async (req, res) => {
  ...
  const address = await get<AddressRow>('SELECT * FROM addresses WHERE id = ?', [addressId]);
...
addressesRouter.patch('/:id/default', requireAuth, async (req, res) => {
  ...
  const existing = await get<AddressRow>('SELECT * FROM addresses WHERE id = ? AND user_id = ?', [
    addressId,
    userId,
  ]);
...
addressesRouter.delete('/:id', requireAuth, async (req, res) => {
  ...
  const remaining = await all<AddressRow>('SELECT * FROM addresses WHERE user_id = ? ORDER BY created_at ASC', [
    userId,
  ]);
