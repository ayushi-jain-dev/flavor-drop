...
const queryAllSync = (db, sql, params = []) => {
...
const seedSampleData = (db) => {
    const { count } = queryAllSync(db, 'SELECT COUNT(*) as count FROM restaurants')[0] ?? {
        count: 0,
    };
    if (Number(count) > 0) {
        return;
    ...
    for (const restaurant of restaurants) {
        runSync(db, `INSERT INTO restaurants (
    ...
    for (const item of menuItems) {
        runSync(db, `INSERT INTO menu_items (
...
export const all = async (sql, params = []) => {
    ...
    return queryAllSync(db, sql, params);
};
...
export const boolFromRow = (value) => toBoolean(value);
